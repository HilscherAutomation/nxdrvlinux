 ...
 * The GCI provides an interface to exchange acyclic data.
 * \{
 */
 ...
 * \note The <<HIL_GCI_APP_SERVICE_AVAILABLE_FN,HIL_GCI_APP_SERVICE_AVAILABLE_FN>> from the GCI-Application may be called before the function returns.
 ...
 * \param ptPacketHeader  [in] Packet header information for the passed data in pabPacketData. <br>
 *                             Note: This pointer shall point to a DWORD aligned address.
 * \param pabPacketData   [in] Pointer to acyclic data buffer with length ptPacketHeader->ulLen. <br>
 *                             Note: The content is only valid until the function returns. <br>
 *                             Note: This pointer shall point to a DWORD aligned address.
 *
 ...
 *         the GCI-Application shall retry the service later in time (recommend retry time is 5ms). <br><br>
 *         Note: The error code shall only be used by the GCI-Component in case the GCI-Component is not able to accept the service,
 *               e.g. no more resources left to accept service or invalid function arguments. <br><br>
 *         Note: GCI-Component implementation note. The error code shall not be a result from the processing of the service.
 ...
 * On successful return (or during function execution), the GCI-Application will call the <<HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN,HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN>>
 * function to consume the announced service data.
 *
 * \note This callback can be called from any context (except IRQ).
 * \note The service data read function callback (<<HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN,HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN>>) may be called before this function returns.
 ...
 *       to <<HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN,HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN>>.
 *
 ...
 *                             in subsequent calls to the <<HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN,HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN>> function.
 * \param ptPacketHeader  [in] Packet header information for the announced data.
 *
 * \return SUCCESS_HIL_OK in case GCI-Application accepts the service, otherwise an error code is returned. <br><br>
 *         Note: The error code shall only be used by the GCI-Application in case the GCI-Application is not able to accept the service,
 *               e.g. no more resources left to accept service, or ptPacketHeader is NULL. The error code shall not be a result from
 *               the processing of the service. <br>
 *         Note: The content of the packet header (ptPacketHeader) shall not be considered as error code, e.g. invalid packet length
 *               or unknown command. In this case, the service shall be accepted anyway. In case of an indication (*_IND) the error code
 *               shall be passed by the response (*_RES) packet status. <br>
 *         Note: In error case the GCI-Application will not read out the data. The GCI-Component may retry to announce the service, where
 ...
 * <<HIL_GCI_APP_SERVICE_AVAILABLE_FN,HIL_GCI_APP_SERVICE_AVAILABLE_FN>>. Typically, The transaction of service data is completed when all announced data has been
 ...
 *                                       Note: Pointer can also be NULL, in this case the GCI-Component can release the data/abort the transaction
 ...
 *                  1. iteration: <<HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN,HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN>> call with ulReadSize = 128 bytes
 *                                -> function returns with 128 bytes
 *                  2. iteration: <<HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN,HIL_GCI_COMP_SERVICE_READ_PACKET_DATA_FN>> call with ulReadSize = 128 bytes
 ...
 * which command codes it will accept (see <<HIL_GCI_COMP_INTERFACE_INITIALIZE_FN,HIL_GCI_COMP_INTERFACE_INITIALIZE_FN>>).
 */
typedef struct HIL_GCI_COMP_SERVICE_COMMAND_RANGE_Ttag
{
   ...
   * in fragments to the GCI-Component by means of one or multiple calls to <<HIL_GCI_COMP_SERVICE_AVAILABLE_FN,HIL_GCI_COMP_SERVICE_AVAILABLE_FN>>.
   ...
   *
   * Note: The mechanism to provide a service in fragments is GCI-Component specific.
   *       This flag is set to true in rare cases. */
 ...
 * <<HIL_GCI_APP_CYCLIC_PROVIDER_DATA_UPDATE_DONE_FN,HIL_GCI_APP_CYCLIC_PROVIDER_DATA_UPDATE_DONE_FN>> to give control over the provider data area back to the GCI-Application.
 ...
 * \param hComponent [in] Handle of the GCI-Component.
 *
 * \return SUCCESS_HIL_OK in case GCI-Component accepts the update request, otherwise an error code is returned. <br><br>
 *         Note:  If accepted, the GCI-Component calls <<HIL_GCI_APP_CYCLIC_PROVIDER_DATA_UPDATE_DONE_FN,HIL_GCI_APP_CYCLIC_PROVIDER_DATA_UPDATE_DONE_FN>> as soon as provider
 ...
 * <<HIL_GCI_COMP_CYCLIC_PROVIDER_DATA_UPDATE_REQUEST_FN,HIL_GCI_COMP_CYCLIC_PROVIDER_DATA_UPDATE_REQUEST_FN>>.
 *
 ...
 * is eligible to be accessed and processed by the GCI-Component. When the GCI-Component has finished access, it will call
 * <<HIL_GCI_APP_CYCLIC_CONSUMER_DATA_UPDATE_DONE_FN,HIL_GCI_APP_CYCLIC_CONSUMER_DATA_UPDATE_DONE_FN>> to give control over the consumer data area back to the GCI-Application.
 ...
 *
 * \return SUCCESS_HIL_OK in case GCI-Component accepts the update request, otherwise an error code is returned. <br><br>
 *         Note:  If accepted, the GCI-Component calls <<HIL_GCI_APP_CYCLIC_CONSUMER_DATA_UPDATE_DONE_FN,HIL_GCI_APP_CYCLIC_CONSUMER_DATA_UPDATE_DONE_FN>> as soon as consumer
 ...
 * <<HIL_GCI_COMP_CYCLIC_CONSUMER_DATA_UPDATE_REQUEST_FN,HIL_GCI_COMP_CYCLIC_CONSUMER_DATA_UPDATE_REQUEST_FN>>.
 *
 * \note This function has to be implemented by the GCI-Application and is called by the GCI-Component.
 *
 * \param hApplication [in] Handle of the GCI-Application.
...
typedef struct HIL_GCI_APP_CYCLIC_INTERFACE_Ttag
{
   ...
   *
      Note: The GCI-Application will be informed about finished consumer data update. */
  HIL_GCI_APP_CYCLIC_CONSUMER_DATA_UPDATE_DONE_FN fnConsumerDataUpdateDone;

  /*! Pointer to consumer data buffer.
   *
   *  Note: The GCI-Application has to ensure to not change the consumer buffer.
   *        Otherwise inconsistent/invalid data might be observed.<br>
   *  Note: This pointer shall point to a DWORD aligned address. */
   ...
   *
   *  Note: The GCI-Application will be informed about finished provider data update. */
   ...
   *
   *  Note: The GCI-Application will be informed about changed consumer/provider data size. */
 ...
 *
 * Request synchronization with the GCI-Component. The GCI-Component will call <<HIL_GCI_APP_SYNC_EVENT_DONE_FN,HIL_GCI_APP_SYNC_EVENT_DONE_FN>>
 ...
 *      The GCI-Component calls back into <<HIL_GCI_APP_SYNC_EVENT_DONE_FN,HIL_GCI_APP_SYNC_EVENT_DONE_FN>> synchronous to the event. The GCI-Application then may
 ...
 *      The GCI-Component will process the event in a best-effort manner and call back into <<HIL_GCI_APP_SYNC_EVENT_DONE_FN,HIL_GCI_APP_SYNC_EVENT_DONE_FN>>
 *      after completion. On the next event, the GCI-Application will call this function again.
 *
 * \note <<HIL_GCI_COMP_SYNC_EVENT_REQUEST_FN,HIL_GCI_COMP_SYNC_EVENT_REQUEST_FN>> and <<HIL_GCI_APP_SYNC_EVENT_DONE_FN,HIL_GCI_APP_SYNC_EVENT_DONE_FN>> must always be called in a well-formed
 *       successive fashion.
 * \note The function is implemented by the GCI-Component and is called by the GCI-Application.
 *
 * \param hComponent [in] Handle of the GCI-Component.
 *
 * \return SUCCESS_HIL_OK in case GCI-Component accepts the request, otherwise an error code is returned. <br><br>
 *         Note: If accepted, the GCI-Component calls <<HIL_GCI_APP_SYNC_EVENT_DONE_FN,HIL_GCI_APP_SYNC_EVENT_DONE_FN>> after event has been processed.
*/
 ...
 * which was triggered by the GCI-Application by means of <<HIL_GCI_COMP_SYNC_EVENT_REQUEST_FN,HIL_GCI_COMP_SYNC_EVENT_REQUEST_FN>>.
 *
...
typedef struct HIL_GCI_APP_SYNC_INTERFACE_Ttag
{
  ...
  /*! Pointer to synchronization handshake error counter.
      ...
      via <<HIL_GCI_APP_SYNC_EVENT_DONE_FN,HIL_GCI_APP_SYNC_EVENT_DONE_FN>>. The value is incremented by one for each Synchronization cycle in
...
typedef struct HIL_GCI_COMP_INFO_Ttag
{
   ...
   *
   * Note: The remanent data size shall be exact the size used during HIL_STORE_REMANENT_DATA_IND. */
 ...
 * using the previously exchanged interfaces (see <<HIL_GCI_COMP_INTERFACE_INITIALIZE_FN,HIL_GCI_COMP_INTERFACE_INITIALIZE_FN>>).

 * \param hComponent [in] Handle of the GCI-Component. Obtained with GCI-Component's interface,
 *                        see <<HIL_GCI_COMP_INTERFACE_INITIALIZE_FN,HIL_GCI_COMP_INTERFACE_INITIALIZE_FN>>.
 */
...
typedef struct HIL_GCI_APP_INTERFACE_Ttag
{
   ...
   *
   * Note: The communication data flow is unidirectional (GCI-Component -> GCI-Application).
   *       The structure, semantics, size and times of access are specific to the GCI-Component
   ...
   *  In case the size is not supported by the GCI-Component, the <<HIL_GCI_COMP_INTERFACE_INITIALIZE_FN,HIL_GCI_COMP_INTERFACE_INITIALIZE_FN>>
...
typedef __HIL_PACKED_PRE struct __HIL_PACKED_POST HIL_GCI_SET_COMMUNICATION_STATUS_IND_DATA_Ttag
{
   ...
   *                                    is activated
   *
   * Note: Depending on the GCI-ProtocolStack, not all of the above states are available or may deviate in meaning.
   */
   ...
   *
   * Note: Even if the GCI-ProtocolStack reports "Communicating" the process data may not be valid. E.g.
   ...
   *       provide a dedicated validation information together with the process data. <br>
   * Note: On slave systems it is highly recommended that the process data which is generated by the
   ...
   *
   * Note: Depending on protocol the communication on the network is still active,
 ...
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_COMMUNICATION_STATUS_IND, HIL_GCI_SET_COMMUNICATION_STATUS_IND>>
 * \param ulLen 14
 */
 ...
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_COMMUNICATION_STATUS_RES, HIL_GCI_SET_COMMUNICATION_STATUS_RES>>
 * \param ulLen 0
 * */
 ...
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_MASTER_STATUS_IND, HIL_GCI_SET_MASTER_STATUS_IND>>
 * \param ulLen 12
 */
 ...
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_MASTER_STATUS_RES, HIL_GCI_SET_MASTER_STATUS_RES>>
 * \param ulLen 0
 */
...
/*! Set process data update interval indication
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_IO_MIN_UPDATE_INTERVAL_IND, HIL_GCI_SET_IO_MIN_UPDATE_INTERVAL_IND>>
 * \param ulLen 2
 */
...
/*! Set process data update interval response
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_IO_MIN_UPDATE_INTERVAL_RES, HIL_GCI_SET_IO_MIN_UPDATE_INTERVAL_RES>>
 * \param ulLen 0
 */
...
/*! \defgroup HIL_GCI_COM_LED_STATUS_IND_CCS CC-Link Slave status indications
* \{*/

/* Definitions if RUN and ERROR LEDs are visually separated from each other */
#define HIL_GCI_COM_LED_STATUS_CCS_RUN_NOT_CONNECTED                                        0x0701 /*!< Not connected */
#define HIL_GCI_COM_LED_STATUS_CCS_RUN_CONNECTED                                            0x0702 /*!< Connected */

#define HIL_GCI_COM_LED_STATUS_CCS_ERROR_NO_ERROR                                           0x0710 /*!< No error */
#define HIL_GCI_COM_LED_STATUS_CCS_ERROR_CRC_ERROR                                          0x0720 /*!< CRC error */
#define HIL_GCI_COM_LED_STATUS_CCS_ERROR_SWITCH_CHANGE                                      0x0730 /*!< Switch setting change */
#define HIL_GCI_COM_LED_STATUS_CCS_ERROR_INVALID_INITIIAL_SETTINGS                          0x0740 /*!< Invalid initial switch setting */
#define HIL_GCI_COM_LED_STATUS_CCS_ERROR_CCLINK_MASTER_VERSION_INCOMPATIBLE                 0x0750 /*!< Incompatible CC-Link Master version */

/* Definitions if only one STATUS LED exist */
#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_NOT_CONNECTED_NO_ERROR                            0x0711 /*!< Not connected, no error */
#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_NOT_CONNECTED_CRC_ERROR                           0x0721 /*!< Not connected, CRC error */
#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_NOT_CONNECTED_SWITCH_CHANGE                       0x0731 /*!< Not connected, Switch setting change */
#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_NOT_CONNECTED_INITIIAL_SETTINGS                   0x0741 /*!< Not connected, Invalid initial switch setting */
#define HIL_GCI_COM_LED_STATUS_CCS_STAUTS_NOT_CONNECTED_CCLINK_MASTER_VERSION_INCOMPATIBLE  0x0751 /*!< Not connected, incompatible CC-Link Master version */

#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_CONNECTED_NO_ERROR                                0x0712 /*!< Connected, no error */
#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_CONNECTED_CRC_ERROR                               0x0722 /*!< Connected, CRC error */
#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_CONNECTED_SWITCH_CHANGE                           0x0732 /*!< Connected, Switch setting change */
#define HIL_GCI_COM_LED_STATUS_CCS_STATUS_CONNECTED_INVALID_INITIIAL_SETTINGS               0x0742 /*!< Connected, Invalid initial switch setting */
#define HIL_GCI_COM_LED_STATUS_CCS_STAUTS_CONNECTED_CCLINK_MASTER_VERSION_INCOMPATIBLE      0x0752 /*!< Connected, incompatible CC-Link Master version */
/*! \} */
 ...
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_COM_LEDS_IND, HIL_GCI_SET_COM_LEDS_IND>>
 * \param ulLen 16
 * */
 ...
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_COM_LEDS_RES, HIL_GCI_SET_COM_LEDS_RES>>
 * \param ulLen 0
 */
...
typedef __HIL_PACKED_PRE struct __HIL_PACKED_POST HIL_GCI_SET_STATE_FIELD_IND_DATA_Ttag
{
   ...
   *
   * Note: The value shall always be greater than zero. */
...
/*! State information field indication
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_STATE_FIELD_IND, HIL_GCI_SET_STATE_FIELD_IND>>
 * \param ulLen 4 + 8 * 'ulNumDataElements'
 */
...
/*! State information field response
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_SET_STATE_FIELD_RES, HIL_GCI_SET_STATE_FIELD_RES>>
 * \param ulLen 0
 */
...
/*! Reset state information field indication
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_RESET_STATE_FIELDS_IND, HIL_GCI_RESET_STATE_FIELDS_IND>>
 * \param ulLen 0
 */
...
/*! Reset state information field response
 *
 * \param ulDest 0x20
 * \param ulCmd <<HIL_GCI_RESET_STATE_FIELDS_RES, HIL_GCI_RESET_STATE_FIELDS_RES>>
 * \param ulLen 0
 */
